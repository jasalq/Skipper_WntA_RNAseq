...
set_merian_colour_mapping <- function(location_set){ # Set mapping of Merian element to colour when only plot 
  ...
  colour_palette <- c("#99403B", "#E78C6D", "#CB8F3B", "#F7E18F",
    "#7D783A", "#CCDC5B", "#E7FFB8", "#79AF39",
    "#A0EF79", "#3F6336", "#4ACF4B", "#9EFBB2",
    "#399257", "#65E6AB", "#CCFFF2", "#3EBEAE",
    "#86ECF6", "#366976", "#53A2DA", "#B1CDFC",
    "#3E56A0", "#7075EE", "#6B43CD", "#C899F7",
    "#693D84", "#CE5CE5", "#FFC2FC", "#B73BA2",
    "#F082C7", "#6A3951", "#D94A74", "#FCA8AF", 'grey')
...
paint_merians_differences_only <- function(spp_df, subset_merians, num_col, title, karyotype){
  ...
  the_plot <- ggplot(data = spp_df) +
    ...
    facet_wrap(query_chr_f ~., ncol=num_col) + guides(scale="right") + 
    xlab("Position (Mb)") +
